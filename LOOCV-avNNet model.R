...
model <- train(factor(Group) ~ CORO1C.creatinine + RAD23B.creatinine + GSPT2.creatinine + NDN.creatinine, data = dat, method = "avNNet", trControl = ctrl, metric = "ROC")
print(model)

# Train model with four proteins plus CEA
model_plusCEA <- train(factor(Group) ~ CORO1C.creatinine + RAD23B.creatinine + GSPT2.creatinine + NDN.creatinine + CEA..ng.ml., data = dat, method = "avNNet", trControl = ctrl, metric = "ROC")
print(model_plusCEA)
